...
class _ActiveStudyView extends ConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    final notifier = ref.read(studySessionControllerProvider(deckId).notifier);
      ...
      try {
        ...
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Could not reveal answer: $error')),
          );
        }
      }
    }
    ...
    Future<void> rateCard(StudyRating rating) async {
      try {
        await notifier.rateCurrent(rating);
      } catch (error) {
        if (context.mounted) {
          ScaffoldMessenger.of(context).showSnackBar(
    ...
    return CallbackShortcuts(
      bindings: {
        ...
        const SingleActivator(LogicalKeyboardKey.digit1): () {
          if (state.isShowingAnswer) {
            rateCard(StudyRating.again);
          }
        },
        const SingleActivator(LogicalKeyboardKey.digit2): () {
          if (state.isShowingAnswer) {
            rateCard(StudyRating.hard);
          }
        },
        const SingleActivator(LogicalKeyboardKey.digit3): () {
          if (state.isShowingAnswer) {
            rateCard(StudyRating.good);
          }
        },
        const SingleActivator(LogicalKeyboardKey.digit4): () {
          if (state.isShowingAnswer) {
            rateCard(StudyRating.easy);
          }
        },
      },
      child: Focus(
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      scheduler.dueReasonLabel(card, DateTime.now()),
                      style: Theme.of(context).textTheme.bodySmall,
                    ),
                    const SizedBox(height: AppSpacing.md),
                    Expanded(
                          ...
                          Wrap(
                            ...
                            children: [
                              _RatingButton(
                                ...
                                onPressed: () => rateCard(StudyRating.again),
                              ),
                              _RatingButton(
                                label: 'Hard',
                                subtitle: scheduler.previewIntervalLabel(
                                ...
                                onPressed: () => rateCard(StudyRating.hard),
                              ),
                              _RatingButton(
                                label: 'Good',
                                subtitle: scheduler.previewIntervalLabel(
                                ...
                                onPressed: () => rateCard(StudyRating.good),
                              ),
                              _RatingButton(
                                label: 'Easy',
                                subtitle: scheduler.previewIntervalLabel(
                                ...
                                onPressed: () => rateCard(StudyRating.easy),
                              ),
                            ],
                          ),
                        ],
